...
def invoke_wasm_function(f: WasmFunctionInstance, module: WasmModule, store: WasmStore, stack: List[WASM_VALUE]):
    if f.functype.argument_types:
        argl = len(f.functype.argument_types)
        args = stack[-argl:]
        stack[-argl:] = []
    else:
        args = []
    rettype = f.functype.return_types
